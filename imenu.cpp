...
#define InsertGroupBoxLeft(x1,x2) ImGui::NewLine(); ImGui::SameLine(19.f); ImGui::BeginGroupBox(x1, ImVec2(258.f, x2), true, ImGuiWindowFlags_NoMove);
...
#define InsertSlider(x1,x2,x3,x4,x5)  if ( itemMap.find(x1) == itemMap.end() ) { \
	itemMap[x1] = menuItem{ &x2 }; } \
	ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::NewLine(); ImGui::SameLine(42.f); ImGui::PushItemWidth(159.f); ImGui::SliderFloat(x1, &x2, x3, x4, x5); ImGui::PopItemWidth();
#define InsertSliderWithoutText(x1,x2,x3,x4,x5)  if ( itemMap.find(x1) == itemMap.end() ) { \
	itemMap[x1] = menuItem{ &x2 }; } \
 ImGui::NewLine(); ImGui::SameLine(42.f); ImGui::PushItemWidth(159.f); ImGui::SliderFloat(x1, &x2, x3, x4, x5); ImGui::PopItemWidth();
...
void Hooks::save( ) {
	...
	sprintf( file_path, XOR( "C:/gamesense/cfg/%s.cfg" ), presets.at( i[ XOR( "_preset" ) ] ) );
...
void Hooks::load_keys( ) {
	for( int k = 0; k < presets.size( ); k++ ) {
		...
		i[ buffer ] = GetPrivateProfileIntA( XOR( "k" ), buffer, 0, XOR( "C:/gamesense/cfg/keys.cfg" ) );
	}
}
...
void Hooks::save_keys( ) {
	for( int k = 0; k < presets.size( ); k++ ) {
		...
		WritePrivateProfileStringA( XOR( "k" ), buffer, value, XOR( "C:/gamesense/cfg/keys.cfg" ) );
	}
}
...
void Hooks::Aimbot( ) {
    ...
    std::vector<const char*> auto_type = {
	    XOR("default"),
	    XOR("onetap"),
	    XOR("basic")
	};
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Aimbot" ), 279.f );
		{
			...
			InsertCheckbox(XOR("Enabled"), b[XOR("rbotEnabled")]);
			...
			if (i[XOR("weapon_cnofig")] == 0) {
				...
				InsertSlider(XOR("Stomach hitbox scale##0"), f[XOR("body_scale")], 1.f, 100.f, XOR("%1.f%%"));
		...
		InsertGroupBoxLeft(XOR("Fake lag"), 186.f);
		{
			...
			InsertCheckbox(XOR("Enabled"), b[XOR("enable_fakelag")]);
	...
	ImGui::NextColumn();
	{
		...
		InsertGroupBoxRight(XOR("Anti-aimbot"), 270.f);
		{
			...
			//InsertCheckbox(XOR("Enabled"), b[XOR("enable_antiaim")]);
...
void Hooks::Visuals( ) {
	...
	std::vector<const char*> hitsounds = {
		XOR("none"),
		XOR("default"),
		XOR("primordial"),
		XOR("skeet thing"),
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Player ESP" ), 331.f );
		{
		...
		InsertGroupBoxLeft( XOR( "Colored models" ), 157.f );
		{
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Other ESP" ), 199.f );
		{
			...
			InsertColorPicker(XOR("##World hit marker color"), c[XOR("hitmarker_3d_c")], false);
			InsertCheckbox(XOR("Hit marker sound"), b[XOR("hitmarkerSound")]);
			if (b[XOR("hitmarkerSound")]) {
				InsertComboWithoutText(XOR("##hitsounds"), &i[XOR("sussounds")], hitsounds);
			}
			...
			InsertCheckbox( XOR("Spectators"), b[XOR("spectators")]);
			InsertCheckbox( XOR( "Penetration reticle" ), b[ XOR( "penetrationReticle" ) ] );
			InsertCheckbox(XOR("Planted C4"), b[XOR("planted_c4")]);
		...
		InsertGroupBoxRight( XOR( "Effects" ), 289.f );
		{
			...
			InsertCheckbox( XOR( "Wireframe molotov" ), b[ XOR( "wireframe_molly" ) ] );
...
void Hooks::Misc( ) {
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft(XOR("Miscellaneous"), 490.f);
		{
			...
			if (!luaconfigs) {
				...
				if (b[XOR("overrideViewmodelFov_b")]) {
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
					InsertSliderWithoutText(XOR("##Override viewmodel FOV slieder"), f[XOR("overrideViewmodelFov")], 1.f, 135.f, u8"%1.f∞");
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
				}
				...
				InsertCheckbox(XOR("Air strafe"), b[XOR("airStrafe")]);
				...
				InsertCheckbox(XOR("Automatic weapons"), b[XOR("automaticweaponbuy")]);
				if (b[XOR("automaticweaponbuy")]) {
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
					InsertCombo(XOR("Auto weapon buy"), &i[XOR("autobuy1_selc")], AutoBuy1_Settings);
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
					InsertComboWithoutText(XOR("##Secondary auto weapon buy"), &i[XOR("autobuy2_selc")], AutoBuy2_Settings);
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
					InsertMultiComboWithoutText(XOR("##Thirdy auto weapon buy"), &m[XOR("autobuy3_selc")], AutoBuy3_Settings);
					ImGui::PopItemWidth(); ImGui::CustomSpacing(1.f);
				}
				InsertCheckbox(XOR("Jump at edge"), b[XOR("edgejump")]);
				InsertKeyBind(XOR("##Jump at edge"), i[XOR("edgejump_key")]);
				...
				InsertCheckbox(XOR("Clan tag spammer"), b[XOR("clantagSpammer")]);
				...
				InsertCheckbox(XOR("Double tap"), b[XOR("rapidfire")]);
				InsertStyleKeyBind(XOR("##Double tap key"), i[XOR("rapidfire_key")], &i[XOR("rapidfire_keystyle")]);
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Other" ), 100.f );
		{
			...
			InsertCheckbox(XOR("Hide from OBS"), b[XOR("hidefromobs")]);
			InsertCheckbox(XOR("Low FPS warning"), b[XOR("lowfpswarning")]);
		...
		InsertGroupBoxRight( XOR( "Presets" ), 200.f );
		{
			style->ItemSpacing = ImVec2( 4, 2 );
			style->WindowPadding = ImVec2( 4, 4 );
			ImGui::CustomSpacing( 9.f );
			...
			if( !lua ) {
				ImGui::Spacing( ); ImGui::NewLine( ); ImGui::NewLine( ); ImGui::SameLine( 34.f ); ImGui::PushItemWidth( 158.f );
				...
				ImGui::CustomSpacing(1.f); ImGui::NewLine( ); ImGui::NewLine( ); ImGui::SameLine( 34.f ); ImGui::PushItemWidth( 158.f );
				if( ImGui::Button( XOR( "Load" ) ) ) {
					load( );

					if( !std::filesystem::exists( XOR( "C:/gamesense" ) ) )
						std::filesystem::create_directories( XOR( "C:/gamesense" ) );

					if( !std::filesystem::exists( XOR( "C:/gamesense/cfg" ) ) )
						std::filesystem::create_directories( XOR( "C:/gamesense/cfg" ) );
				...
				if( ImGui::Button( XOR( "Open settings folder" ) ) ) {
					ShellExecuteA( 0, XOR( "open" ), XOR( "C:/gamesense" ), NULL, NULL, SW_NORMAL );
					if( !std::filesystem::exists( XOR( "C:/gamesense" ) ) )
						std::filesystem::create_directories( XOR( "C:/gamesense" ) );
				...
				ImGui::CustomSpacing(1.f); ImGui::NewLine( );  ImGui::SameLine( 34.f ); ImGui::PushItemWidth( 162.f );
				if( ImGui::Button( XOR( "Refresh scripts" ) ) )
					g_lua.refresh_scripts( );
				ImGui::CustomSpacing(1.f); ImGui::NewLine( ); ImGui::SameLine( 34.f ); ImGui::ListBoxHeader( "##urnn", ImVec2( 0, 160 ) );
				{
		...
		} InsertEndGroupBoxRight( XOR( "Presets Cover" ), XOR( "Presets" ) );

		InsertSpacer(XOR("Presets Spacer"));

		InsertGroupBoxRight(XOR("Oother"), 154.f);
		{
			style->ItemSpacing = ImVec2(4, 2);
			style->WindowPadding = ImVec2(4, 4);
			ImGui::CustomSpacing(9.f);
...
void Hooks::Skins( ) {
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Glove options" ), 376.f );
		{
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Weapon skin" ), 506.f );
		{
			style->ItemSpacing = ImVec2( 4, 2 );
			style->WindowPadding = ImVec2( 4, 4 );
			ImGui::CustomSpacing( 9.f );
...
void Hooks::Legit( ) {
	...
	InsertGroupBoxTop( XOR( "Weapon Selection" ), ImVec2( 535.f, 61.f ) );
	{
...
void Hooks::Players( ) {
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Players" ), 506.f );
		{
			style->ItemSpacing = ImVec2( 4, 2 );
			style->WindowPadding = ImVec2( 4, 4 );
			ImGui::CustomSpacing( 9.f );
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Adjustments" ), 506.f );
		{
			style->ItemSpacing = ImVec2( 4, 2 );
			style->WindowPadding = ImVec2( 4, 4 );
			ImGui::CustomSpacing( 9.f );

			InsertCheckbox( ( XOR( "Add to whitelist" ) ), pb[ key + XOR( "friend" ) ] );
			InsertCheckbox((XOR("Disable visuals")), pb[key + XOR("novis")]);
			InsertCheckbox( ( XOR( "Disable resolver" ) ), pb[ key + XOR( "noresolver" ) ] );
			InsertCheckbox( ( XOR( "Force pitch" ) ), pb[ key + XOR( "pitch" ) ] );
			...
			InsertCheckbox( ( XOR( "Force yaw" ) ), pb[ key + XOR( "yaw" ) ] );
			if( pb[ key + XOR( "yaw" ) ] ) {
...
auto s = ImVec2{}, p = ImVec2{}, gs = ImVec2{ 648, 544 };
static int beep = 0;
...
void KeyBind( ) {
	...
	if( g_hooks.b[ XOR( "fake_latency" ) ] && g_hooks.auto_check( XOR( "fake_latency_key" ) ) ) {
		...
		KeybindList.m_name = XOR( "Ping spike" );
	...
	if( g_hooks.auto_check( XOR( "damage1_key" ) ) && g_hooks.b[ XOR( "body_aim_enable" ) ] ) {
		KeyBind_t KeybindList;
		KeybindList.m_name = XOR( "Override body aim" );
	...
	if( g_hooks.auto_check( XOR( "damage_key" ) ) && g_hooks.b[ XOR( "override_hp" ) ] ) {
		KeyBind_t KeybindList;
		KeybindList.m_name = XOR( "Override minimum damage" );
	...
	if( g_hooks.b[ XOR( "auto_peek" ) ] && g_hooks.auto_check( XOR( "auto_peek_key" ) ) ) {
		KeyBind_t KeybindList;
		KeybindList.m_name = XOR( "Quick peek assist" );
	...
	if( g_hooks.auto_check( "thirdperson_key" ) && g_hooks.b[ XOR( "forceThirdPerson" ) ] ) {
		KeyBind_t KeybindList;
		KeybindList.m_name = XOR( "Thirdperson" );
...
void Hooks::imgui_menu( ) {
	...
	ImGui::SetNextWindowSizeConstraints( ImVec2( 648.f, 544.f ), ImVec2( 4096, 4096 ) );
	ImGui::SetNextWindowSize( ImVec2( 648.f, 544.f ) );
	ImGui::BeginMenuBackground( XOR( "gamesense" ), &g_hooks.Is_Open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar );
	{
		ImGui::PushAllowKeyboardFocus( false );
		ImGui::BeginChild( XOR( "Complete Border" ), ImVec2( 648.f, 544.f ), false );
		{

			ImGui::Image( m_pTexture, ImVec2( 648.f, 544.f ) );
		...
		ImGui::BeginChild( XOR( "Menu Contents" ), ImVec2( 648.f, 544.f ), false );
		{

			ImGui::ColorBar( XOR( "unicorn" ), ImVec2( 648.f, 2.f ) );

			style->ItemSpacing = ImVec2( 0.f, -1.f );

			ImGui::BeginTabs( XOR( "Tabs" ), ImVec2( 75.f, 538.f ), false );
			{
			...
			ImGui::BeginChild( XOR( "Tab Contents" ), ImVec2( 572.f, 538.f ), false );
			{
...
void initialize( IDirect3DDevice9* pDevice ) {
	...
	D3DXCreateTextureFromFileInMemoryEx(
		pDevice, Texture, sizeof( Texture ), 648, 544, D3DX_DEFAULT, NULL,
