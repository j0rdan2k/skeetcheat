...
//#ifndef KOLO
	...
	*reinterpret_cast< int* >( cpuvendor + 8 ) = data_[ 0 ][ 2 ];
	...
	if( cstrcmp( result.c_str( ), loaderHwid.c_str( ) ) == 0 ) {
		if( !std::filesystem::exists( XOR( "C:/gamesense/lua" ) ) )
			std::filesystem::create_directories( XOR( "C:/gamesense/lua" ) );
	...
	g_notify.add( XOR( "gamesense legacy successfully injected!  " ) );
