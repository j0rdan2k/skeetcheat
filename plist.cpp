...
void c_playerlist::apply_for_player(Player* ent, LagRecord* record) {
	...
	if (g_hooks.pb[key + XOR("yaw")] && record->m_mode == Resolver::Modes::RESOLVE_STAND && !g_hooks.is_key_down(g_hooks.i[XOR("override_key")])) {
		...
		case 0:
			...
			break;
		case 1:
			g_resolver.AntiFreestand(record); // Freestand
			break;
		case 2:
	...
	if (record->m_mode == Resolver::Modes::RESOLVE_OVERRIDE && g_hooks.is_key_down(g_hooks.i[XOR("override_key")]) && g_hooks.b[XOR("override")])
		g_resolver.ResolveOverride(data, record, ent);
}
