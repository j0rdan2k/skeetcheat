...
void Visuals::ParticalMolly() {
	...
	// this is just suprem wireframe smoke just with the fire particles
	std::vector<const char*> vistasmoke_mats =
	{
	...
	for (auto mat_s : vistasmoke_mats)
	{
		...
		if (g_hooks.b[XOR("wireframe_molly")]) {
			mat->SetFlag(MATERIAL_VAR_WIREFRAME, true);
		}
		else {
			mat->SetFlag(MATERIAL_VAR_MODEL, true);
			mat->SetFlag(MATERIAL_VAR_WIREFRAME, false);
		}
	}
}
...
void Visuals::AutomaticPeekIndicator() {
	...
	static float auto_peek_radius = 0;
	bool condition = g_hooks.b[XOR("auto_peek")] && g_hooks.auto_check(XOR("auto_peek_key")) || alpha;
	float multiplier = g_hooks.f[XOR("auto_peek_anim_speed")] * g_csgo.m_globals->m_frametime;
	...
	auto_peek_radius = std::clamp(auto_peek_radius, 0.0f, 1.0f);
	...
	if (g_hooks.b[XOR("auto_peek_ind")]) {
		if (g_hooks.b[XOR("auto_peek")] && g_hooks.auto_check(XOR("auto_peek_key")) || alpha) {
			...
			if (g_hooks.auto_check(XOR("auto_peek_key")))
				...
				alpha -= 3.0f * g_csgo.m_globals->m_curtime; // NIGGER
			...
			if (g_hooks.b[XOR("auto_peek_anim")]) {
				fart = auto_peek_radius;
			}
			else {
				fart = 1.f;
			}
			...
			if (auto_peek_radius > 0.f) {
				...
				switch (g_hooks.i[XOR("autopeek_types")] % 4) {

				case 0: render::Draw3DFilledCircle(position, 15.f * fart, g_movement.fired_shot ? Color(g_hooks.c["auto_peek_active"][0] * 255.0f, g_hooks.c["auto_peek_active"][1] * 255.0f, g_hooks.c["auto_peek_active"][2] * 255.0f, (int)(alpha * 255.0f)) : Color(g_hooks.c["auto_peek_inactive"][0] * 255.0f, g_hooks.c["auto_peek_inactive"][1] * 255.0f, g_hooks.c["auto_peek_inactive"][2] * 255.0f, (int)(alpha * 255.0f))); break;
				case 1: render::sphere2(position, 15.f * fart, 1, 1, g_movement.fired_shot ? Color(g_hooks.c["auto_peek_active"][0] * 255.0f, g_hooks.c["auto_peek_active"][1] * 255.0f, g_hooks.c["auto_peek_active"][2] * 255.0f, (int)(alpha * 255.0f)) : Color(g_hooks.c["auto_peek_inactive"][0] * 255.0f, g_hooks.c["auto_peek_inactive"][1] * 255.0f, g_hooks.c["auto_peek_inactive"][2] * 255.0f, (int)(alpha * 150.0f))); break;
				case 2: render::sphere1(position, 15.f * fart, 1, 1, g_movement.fired_shot ? Color(g_hooks.c["auto_peek_active"][0] * 255.0f, g_hooks.c["auto_peek_active"][1] * 255.0f, g_hooks.c["auto_peek_active"][2] * 255.0f, (int)(alpha * 255.0f)) : Color(g_hooks.c["auto_peek_inactive"][0] * 255.0f, g_hooks.c["auto_peek_inactive"][1] * 255.0f, g_hooks.c["auto_peek_inactive"][2] * 255.0f, (int)(alpha * 150.0f))); break;

				}
			}
			//render::Draw3DFilledCircle( position, auto_peek_radius, g_movement.fired_shot ? Color( g_hooks.c[ "auto_peek_active" ][ 0 ] * 255.0f, g_hooks.c[ "auto_peek_active" ][ 1 ] * 255.0f, g_hooks.c[ "auto_peek_active" ][ 2 ] * 255.0f, ( int )( alpha * 255.0f ) ) : Color( g_hooks.c[ "auto_peek_inactive" ][ 0 ] * 255.0f, g_hooks.c[ "auto_peek_inactive" ][ 1 ] * 255.0f, g_hooks.c[ "auto_peek_inactive" ][ 2 ] * 255.0f, ( int )( alpha * 255.0f ) ) );
		}
	}
}
...
void Visuals::Spectators( ) {
	...
	for( size_t i{ }; i < spectators.size( ); ++i ) {
		...
		render::esp.string( g_cl.m_width - 15, ( i * 15 ) - 10, { 220, 220, 220, 179 }, name, render::ALIGN_RIGHT );
	}
}
...
void Visuals::StatusIndicators( ) {
	...
	*/
	...
	if (g_hooks.auto_check(XOR("damage_key")) && g_hooks.b[XOR("override_hp")]) {
		...
		sussy = g_hooks.f[XOR("baim_hp_override")];
		dmg = tfm::format(XOR("%s"), sussy);

		std::string bomb_text = tfm::format(XOR("%i"), dmg);

		ind.color = colors::white;
		ind.text = XOR(bomb_text);
	...
	if( g_hooks.auto_check( XOR( "rapidfire_key" ) ) && g_hooks.b[ XOR( "rapidfire" ) ] ) {
		Indicator_t ind{ };
		ind.color = g_cl.m_charged ? Color(255, 255, 255, 255) : 0xff0000ff;
		ind.text = XOR( "DT" );
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::FontSize_t size = render::hud.size(indicator.text);
		render::indicator.string(15, g_cl.m_height - 64 - (30 * i) - g_hooks.f[XOR("keybinds_y")], (0, 0, 0, 1), indicator.text);
		render::indicator.string(15, g_cl.m_height - 64 - (30 * i) - g_hooks.f[XOR("keybinds_y")], indicator.color, indicator.text);
	}
}
...
void Visuals::DrawPlantedC4( ) {
	...
	mode_2d = (!g_hooks.b[XOR("planted_c4")]);
