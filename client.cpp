...
void Client::DrawHUD() {
	...
	// colors
	const auto col_background = Color(5, 5, 5, 255);
	...
	std::string text = tfm::format(XOR("gamesense | %s | delay: %ims | %s"), g_cl.m_user, ms, time.str().data());
	...
	render::round_rect(m_width - size.m_width - 24, 12, size.m_width + 15, size.m_height + 11, 1, { 34, 60, 60, 255 }); // обводка мб
	render::round_rect(m_width - size.m_width - 24, 12, size.m_width + 15, size.m_height + 11, 1, { 40, 40, 40, 255 }); // вверхняя
	render::round_rect(m_width - size.m_width - 24, 15, size.m_width + 15, size.m_height + 11, 1, { 34, 60, 60, 255 }); // обводка мб
	render::round_rect(m_width - size.m_width - 24, 15, size.m_width + 15, size.m_height + 11, 1, { 40, 40, 40, 255 }); // нижния полоса

	render::rect_filled(m_width - size.m_width - 20, 15, size.m_width + 7, size.m_height + 8, col_background);

	// lbgt line
	render::gradient1337(m_width - size.m_width - 20, 18, size.m_width + 6, 1, { 99, 160, 200, 255 }, { 179, 102, 181, 255 }, 1);
	render::gradient1337(m_width - size.m_width - 20, 18, size.m_width + 6, 1, { 179, 102, 181, 255 }, { 230, 217, 100, 255 }, 1);
	render::gradient1337(m_width - size.m_width - 20, 18, size.m_width + 6, 1, { 49, 79, 99, 130 }, { 89, 50, 90, 130 }, 1);
	render::gradient1337(m_width - size.m_width - 20, 18, size.m_width + 6, 1, { 89, 50, 90, 130 }, { 114, 108, 49, 130 }, 1);

	// text.
	render::menu_shade.string(m_width - 16, 21, { 255, 255, 255, 255 }, tfm::format(XOR(" | %s | delay: %ims | %s"), g_cl.m_user, ms, time.str().data()), render::ALIGN_RIGHT);
	render::menu_shade.string(m_width - 16 - render::menu_shade.size(tfm::format(XOR(" | %s | delay: %ims | %s"), g_cl.m_user, ms, time.str().data())).m_width, 21, col_accent, tfm::format(XOR("sense")), render::ALIGN_RIGHT);
	render::menu_shade.string(m_width - 16 - render::menu_shade.size(tfm::format(XOR("sense | %s | delay: %ims | %s"), g_cl.m_user, ms, time.str().data())).m_width, 21, { 240, 240, 240, 200 }, tfm::format(XOR("game")), render::ALIGN_RIGHT);
}
...
void Client::OnTick( CUserCmd* cmd ) {
	...
	if( g_hooks.auto_check( XOR( "rapidfire_key" ) ) && g_hooks.b[ XOR( "rapidfire" ) ] ) {
		if( !g_aimbot.CanDT( ) )
			m_charged = false;
		else if( !m_charged && m_tick_to_recharge == 0 && game::TIME_TO_TICKS(.5) ) {
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( colors::green, XOR( "[gamesense] " ) );
...
void Client::ClanTag( ) {
	...
	// gamesense.
	if (g_hooks.b[XOR("clantagSpammer")]) {
		// are we in a new frame?
		if (iPrevFrame != (int)(g_csgo.m_globals->m_curtime * 2.9f) % 23) {
			switch (int(g_csgo.m_globals->m_curtime * 2.9f) % 23) {
			case 0: SetClanTag(XOR("              ")); break;
			case 1: SetClanTag(XOR("             g")); break;
			case 2: SetClanTag(XOR("            ga")); break;
			case 3: SetClanTag(XOR("           gam")); break;
			case 4: SetClanTag(XOR("          game")); break;
			case 5: SetClanTag(XOR("         games")); break;
			case 6: SetClanTag(XOR("        gamese")); break;
			case 7: SetClanTag(XOR("       gamesen")); break;
			case 8: SetClanTag(XOR("      gamesens")); break;
			case 9: SetClanTag(XOR("     gamesense")); break;
			case 10:SetClanTag(XOR("    gamesense ")); break;
			case 11:SetClanTag(XOR("   gamesense  ")); break;
			case 12:SetClanTag(XOR("  gamesense   ")); break;
			case 13:SetClanTag(XOR(" gamesense    ")); break;
			case 14:SetClanTag(XOR("gamesense     ")); break;
			case 15:SetClanTag(XOR("amesense      ")); break;
			case 16:SetClanTag(XOR("mesense       ")); break;
			case 17:SetClanTag(XOR("esense        ")); break;
			case 18:SetClanTag(XOR("sense         ")); break;
			case 19:SetClanTag(XOR("ense          ")); break;
			case 20:SetClanTag(XOR("nse           ")); break;
			case 21:SetClanTag(XOR("se            ")); break;
			case 22:SetClanTag(XOR("e             ")); break;
			}

			// set current/last frame.
			iPrevFrame = (int)(g_csgo.m_globals->m_curtime * 2.7f) % 23;
		}
	...
	//	if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 21) {
	//		switch (int(g_csgo.m_globals->m_curtime * 3.6) % 21) {
	//		case 0: {  SetClanTag("[  ]"); break; }
	//		case 1: {  SetClanTag("[ o ]"); break; }
	//		case 2: {  SetClanTag("[ ob ]"); break; }
	//		case 3: {  SetClanTag("[ obl ]"); break; }
	//		case 4: {  SetClanTag("[ obli ]"); break; }
	//		case 5: {  SetClanTag("[ obliv ]"); break; }
	//		case 6: {  SetClanTag("[ oblivi ]"); break; }
	//		case 7: {  SetClanTag("[ oblivio ]"); break; }
	//		case 8: {  SetClanTag("[ oblivion ]"); break; }
	//		case 9: {  SetClanTag("[ 0bl|v|0n ]"); break; }
	//		case 10: { SetClanTag("[ oblivion ]"); break; }
	//		case 11: { SetClanTag("[ 0bl|v|0n ]"); break; }
	//		case 12: { SetClanTag("[ oblivion ]"); break; }
	//		case 13: { SetClanTag("[ blivion ]"); break; }
	//		case 14: { SetClanTag("[ livion ]"); break; }
	//		case 15: { SetClanTag("[ ivion ]"); break; }
	//		case 16: { SetClanTag("[ vion ]"); break; }
	//		case 17: { SetClanTag("[ ion ]"); break; }
	//		case 18: { SetClanTag("[ on ]"); break; }
	//		case 19: { SetClanTag("[ n ]"); break; }
	//		case 20: { SetClanTag("[  ]"); break; }
	//		}

	//		// set current/last frame.
	//		iPrevFrame = (int)(g_csgo.m_globals->m_curtime * 1.9f) % 24;
	//	}
	//}
	else {
		// reset our clantag.
